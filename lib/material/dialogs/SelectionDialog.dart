...
class SelectionDialog<T extends FieldInterface> extends StatefulWidget {
  ...
  @override
  State<SelectionDialog<T>> createState() => SelectionDialogState<T>();
}

class SelectionDialogState<T extends FieldInterface> extends State<SelectionDialog<T>> {
  late Map<T, bool> values = {
    for (var i in widget.items) i: false,
  };
      ...
      actions: [
        if (widget.onConfirm != null)
          IconButton(
            ...
            onPressed: () {
              List<T> list = values.entries.where((e) => e.value == true).map((e) => e.key).toList();
